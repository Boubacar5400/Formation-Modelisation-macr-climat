...
% chap 2.1.2 - labor supply temporary shock
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
//-----------------------------------------
// 9) Export CSV endogenous variables
//-----------------------------------------
    ...
    % === data extraction ===
    X  = oo_.endo_simul';               % -> dimension T×N
    varnames = cellstr(M_.endo_names);  % variables names
    T  = size(X,1);                     % number of periods

    % === Création d’une table (Octave/Matlab compatible) ===
    C = [ {'time'}, varnames' ; num2cell((1:T)'), num2cell(X) ];

    % === file path ===
    outfile = fullfile(outdir, 'endo_simul_LBAR.csv');
    ...
    disp([' Fichier CSV exporté : ', outfile]);
