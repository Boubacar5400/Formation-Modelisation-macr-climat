...
initval;
  ...
  % Valeurs initiales "cohérentes" (peu importe, le SS les écrase)
  xi = xi_bar;
  e  = 0.0;
  s  = 0.0;
end;
...
steady_state_model;
  ...
  xi = xi_bar;
  e  = xi * y;
  s  = e / (1 - (1 - deltae)/gamm);   % = xi*y * gamm / (deltae + gbar)
end;
...
shocks;
  ...
  var tau_y; periods 50:350; values 0.3;

  % (option) trajectoire d'intensité : laisser le processus AR(1) s'ajuster
  % (pas de choc explicite ici)
end;
